...
__global__ void reduce_kernel(DATA_TYPE *mean, DATA_TYPE *data, long M, long N)
{
	int j = blockIdx.x * blockDim.x + threadIdx.x + 1;
...
__global__ void covar_kernel(DATA_TYPE *symmat, DATA_TYPE *data, long M, long N)
{
...
int main(int argc, char *argv[])
{
	...
	M = get_data_size(data_size/2, sizeof(DATA_TYPE), 2);
	N = M;

...
void fdtdCuda(DATA_TYPE* _fict_gpu, DATA_TYPE* ex_gpu, DATA_TYPE* ey_gpu, DATA_TYPE* hz_gpu, int haha)
{
	...
	t_end = rtclock();
	if (haha) fprintf(stdout, "GPU Runtime: %0.3lfs\n", t_end - t_start);
}
...
int main(int argc, char *argv[])
{
	...
	// } else {
	if (!compare_with_cpu)	TOUCH_ARRAY(hz, (NX * NY * sizeof(DATA_TYPE)));
	// }
